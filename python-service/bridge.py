...
BAUD_RATE = 9600
SERVER_URL = "http://localhost:5001"
# --- ADD THE REAL INTERSECTION ID ---
REAL_INTERSECTION_ID = "68bf113329a3d66abae0fd7c" # Must match server/cityState.js
...
def connect():
    print("✅ Successfully connected to the Node.js WebSocket server.")
    # --- Tell the server which intersection this bridge is for ---
    sio.emit('subscribe-to-intersection', REAL_INTERSECTION_ID)
    print(f"📡 Subscribed to room: {REAL_INTERSECTION_ID}")

@sio.event
def connect_error(data):
    print(f"❌ Connection to WebSocket server failed! Data: {data}")
...
def on_arduino_command(data):
    ...
    try:
        ...
        # Optionally, you could try to reconnect here or exit.
